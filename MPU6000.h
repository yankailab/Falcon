...
// - output looks like garbage, for instance
...
public:
	...
	boolean			m_dmpReady;				// set true if DMP initialization was successful
	unsigned int	m_packetSize;			// number of unique bytes of data written by the DMP each time (FIFO can hold multiples of 42-bytes)
	unsigned int	m_fifoCount;			// count of all bytes currently in FIFO
	byte			m_fifoBuffer[64];		// FIFO storage buffer (in fact only 42 used...)

	// INTERRUPT FROM MPU-6000 DETECTION ROUTINE
	boolean			m_mpuInterrupt;			// indicates whether MPU interrupt pin has gone high
private:
